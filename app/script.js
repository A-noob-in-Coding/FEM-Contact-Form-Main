...
function isEmptyInput() {
  for (let i = 0; i < IdsOfInputs.length; i++) {
    var El = document.getElementById(IdsOfInputs[i]) 
    if(El.value === "")
    {
        return true
    }  
...
function consentErrorHandler() {
  const ConsentChecked =
    document.querySelector('input[name="consent"]:checked') !== null;
  if (ConsentChecked) {
